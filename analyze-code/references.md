...
Heavy reference material for the `analyze-code` skill. Load on demand from Step 9 or when a finding needs the detail.

## Tooling discovery paths

When Step 9 runs, search for the project's authoritative command list in this order. First match wins, but check all sources if rules conflict:
...
If a project ships relevant files but none of these scanners is configured or installed, the absence is itself a Medium finding (Step 9 "no tooling configured" rule).
...
- Lockfile **contents** (the file's existence is still a Step 9 signal)
